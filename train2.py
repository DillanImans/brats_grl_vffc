...
def save_best_model(dir_name, model, name="best_model"):
    ...
    # For DDP, model.module holds the actual weights (if wrapped)
...
def train_epoch(model, loader, optimizer, loss_func, device, augment=False):
    """
    ...
    with tqdm(loader, desc="Training", leave=False) as progress_bar:
        for batch_data in progress_bar:
            ...
            run_loss.update(loss.item(), n=image.shape[0])

            progress_bar.set_postfix(loss=f"{run_loss.avg:.4f}")
...
def trainer(
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        train_loss = train_epoch(model, train_loader, optimizer, loss_func, device, augment=False)
...
def main(cfg: DictConfig):
    ...
    if cfg.model.architecture == "unet3d":
        from networks.models.UNet.model import UNet3D
        model = UNet3D(in_channels = 2, num_classes = 3).to(device)
