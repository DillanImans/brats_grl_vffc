...
from myCodeArchives.nnAdapts import conv_nd, conv_nd_class, avg_pool_nd, norm_nd, DropPath

class FourierUnit(nn.Module):
    
    def __init__(self, in_channels, out_channels, groups = 1, fft_norm = 'ortho'):
        # bn layer not used here
...
class SpectralTransform(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dims, stride = 1, groups = 1, enable_lfu = True, **fu_kwargs):
        ...
        self.conv1 = nn.Sequential(
            conv_nd(dims = dims, in_channels = in_channels, out_channels = out_channels // 2, 
    ...
    def forward(self, x):
        ...
        # So we have the normal thing, the global FU, and the local FU in all.
        # So we dont actually lose the 'ol x ting, we just combine stuff tgt.
...
class FFC(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dims, kernel_size,
        ...
        self.convg2g = module(in_cg, out_cg, dims, stride, 1 if groups == 1 else groups // 2,
...
class FFCResnetBlock(nn.Module):
    def __init__(self, dims, dim, padding_type, norm_layer, activation_layer = nn.ReLU,
                 dilation = 1, inline = False, drop_path_rate = 0., **conv_kwargs):
        super().__init__()
...
class Bottleneck(nn.Module):
    def __init__(self, feats_num_bottleneck, norm_layer, n_blocks = 3, padding_type = 'reflect',
                 activation_layer = nn.ReLU, resnet_conv_kwargs = {}, inline = False, drop_path_rate = 0.):
        ...
        for i in range(n_blocks):
            cur_resblock = FFCResnetBlock(3, feats_num_bottleneck, padding_type = padding_type,
                                          activation_layer = activation_layer,
                                          norm_layer = norm_layer, inline = inline,
